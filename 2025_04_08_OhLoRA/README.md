...
* Fine-Tuning 된 StyleGAN 을 이용하여 이미지 생성
  ...
  * 핵심 속성 값 (8가지)
    * 성별, 이미지 품질, 눈을 뜬 정도, 머리 색, 입을 벌린 정도, 배경색의 밝기, 배경색의 표준편차, 고개 돌림
    * 성별, 이미지 품질이 **모두 조건을 충족시키는 이미지만 따로 필터링** 하여, 필터링된 이미지에 대해서만 나머지 6개 속성 값 적용
...
| 이미지 품질 판단 (CNN 구현 포함)            | 04.10 목 (1d)           | ```P002-002-CNN```             | ⬜  |
| 성별 속성 값 추출 (CNN 구현 포함)           | 04.10 목 (1d)           | ```P002-002-CNN```             | ⬜  |
