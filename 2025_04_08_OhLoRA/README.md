...
* Fine-Tuning 된 StyleGAN 을 이용하여 이미지 생성
  ...
  * 핵심 속성 값 (7가지)
    * 성별, 이미지 품질, 눈을 뜬 정도, 머리 색, 머리 길이, 입을 벌린 정도, 고개 돌림
    * 성별, 이미지 품질이 **모두 조건을 충족시키는 이미지만 따로 필터링** 하여, 필터링된 이미지에 대해서만 나머지 5개 속성 값 적용
...
| 핵심 속성 값을 처리하도록 StyleGAN 구조 변경 (Fine-Tuning 포함) | 04.11 금 - 04.12 토 (2d) | ```P002-004-Update-StyleGAN``` | 💨 |
