...
| Computer Vision  | CNN (Conv. NN)                                                                                                                                            | StyleGAN 으로 생성한 가상 인간 이미지 중 성별 값이 있는 2,000 장을 제외한 나머지 이미지들의 **성별 값** 추론                                                                                                                                                                                                         |
...
| 기본 StyleGAN 구현                   | 04.09 - 04.10 목 (2d)   | ```P002-001-StyleGAN```        | 💨 |
| Segmentation 모델 구현 (핵심 속성 값 추출용) | 04.10 목 - 04.11 금 (2d) | ```P002-002-Seg```             | ⬜  |
| Segmentation 으로 핵심 속성 값 추출       | 04.11 금 (1d)           | ```P002-002-Seg```             | ⬜  |
| 성별 속성 값 추출 (CNN 구현 포함)           | 04.11 금 (1d)           | ```P002-003-CNN```             | ⬜  |
| 핵심 속성 값을 처리하도록 StyleGAN 구조 변경    | 04.12 토 (1d)           | ```P002-004-Update-StyleGAN``` | ⬜  |
...
| SFT + LoRA 를 이용한 Fine-Tuning     | 04.14 월 - 04.15 화 (2d) | ```P002-008-SFT-LoRA```        | ⬜  |
| KTO 를 이용한 추가 Fine-Tuning         | 04.15 화 (1d)           | ```P002-009-KTO```             | ⬜  |
| RAG 적용                           | 04.15 화 - 04.16 수 (2d) | ```P002-010-RAG```             | ⬜  |
