...
* ⚡️ 표시는 **ruff [(GitHub)](https://github.com/astral-sh/ruff)** 로 대체된 부분.
* 🔷 표시는 **2026.09.10 이후 신규** 추가한 부분.
...
* [2. PyTorch 관련 (폐기)](#2-pytorch-관련-폐기)
...
* 중복되는 사항들은 common 으로 빼는 것이 좋음
  ...
  * 예) 처음 공통 3줄 이상, 마지막 공통 3줄 이상이 여러 함수에서 동일하거나 매우 유사한 경우 리팩토링 필요
    * 단, 3줄 합산 80바이트 이하 시, 4줄 이상 
...
* 🔮 고정값 (이후 수정되지 않는) 은 코드 중간이 아닌, 맨 위쪽에 상수로 빼 놓는 것이 좋음 (`-1.0`, `0.0`, `1.0`, `-1`, `0`, `1` 을 제외한 숫자 값 한정)
* 🔷🔮 숫자 값을 constant 처럼 사용하는 경우, 해당 값을 맨 위쪽에 상수로 빼는 것이 적절한지 판단 (코드 읽는 시점에서 해당 숫자 값의 의미를 모를 수 있을 때)
* 🔷🔮 `-1.0`, `0.0`, `1.0`, `-1`, `0`, `1`을 제외한 동일한 숫자 값을 2번 이상 사용하는 경우, 상수로 통합해서 값 할당을 맨 위쪽으로 빼는 것이 적절한지 판단 
* ⚡️ 한 줄의 길이는 120 글자 이내여야 함
...
* `if-elif-else` 가 반복되면서 body 패턴이 1줄 정도로 간단하며 모두 동일한 경우 `dict` 를 이용하여 단순화 (조건 분기 남발 자제)
...
* ⚡️ 아래에서 `before`로 된 부분은 `pathlib`을 사용하여 `after`처럼 사용해야 함
  * ⚡️ `read_text` 뿐만 아니라 `write_text` 도 마찬가지 
...
* `itertools.product` 사용 가능한 경우 사용
...
* ⚡️ 파일 열기, 닫기 시 컨텍스트 매니저 방식인 `with open(...)` 사용
...
* ⚡️ `re.sub` 등 정규표현식 사용 시 표현식 string을 `r'...'` 형식으로 사용하도록 함
* ⚡️ `f = lambda x: ...` 보다는 `def f(x): return ...` 를 사용한다.
  * ⚡️ 단, 함수의 인자로 함수를 지정하여 호출하는 경우 제외 
* prefix, suffix 검사 시 `startswith()`, `endswith()` 를 사용한다.

### 1-5. 예외 및 오류 처리

* ⚡️ 예외를 삼키는 경우가 없어야 함
* ⚡️ 예외의 종류 (`OOOError` 등) 를 가급적 구체적으로 명시해야 함
* ⚡️ 함수 관련 오류 방지
  * ⚡️ 함수의 인수를 변경 가능한 default value (예: `data: dict = {...}`) 로 하지 않는다.
* assertion을 `try-except` 등을 이용한 제어 메커니즘으로 사용하지 않는다.
* ⚡️ Python 예약어를 변수명으로 사용하지 않는다.

### 1-6. 코드 응집성, 클래스화 등

* 🔮 다음과 같은 경우, 객체 지향 class 로 만드는 것을 적극 고려
  * 동일한 인수 집합을 갖는 함수가 많은 경우
  * 🔮 상태 값으로 판단되는 값을 조건으로 하여 body가 1줄 정도의 일정한 패턴인 `if-elif-else` 처리가 있는 경우
* ⚡️ 객체의 인터페이스 공개용이 아닌 (= 클래스 내부에서만 쓰이는) 속성, 메서드에 접두사 `_` 있을 때 호출하는 것을 수정
* 🔮 한 모듈 (`*.py` 파일) 내에서, 유사한 이름의 함수끼리 멀리 떨어져 있으면 안됨
  * 예를 들어, `load_gemini_api`, `load_openai_api` 는 서로 유사한 함수
  * 예를 들어, `load_dataset`, `load_final_report` 는 같은 `load` 이지만 코드 전체에서 `load`로 시작하는 함수가 많을 가능성이 있고, 가리키는 대상이 `dataset`, `final_report`로 전혀 다르고 순차적이라고 볼 수도 없으므로 서로 유사하다고 보기 어려움 

## 2. PyTorch 관련 (폐기)

폐기 사유: **추후 PyTorch, TensorFlow 오류 확인 도구 (로컬 AI 기반)** 개발 예정
