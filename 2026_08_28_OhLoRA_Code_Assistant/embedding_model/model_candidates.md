...
* 최종 선정 모델
  ...
  * `LateOn-Code-pretrain` (`Giga-...-0826` 대체 경량화 모델)
...
* 최종 선정 벤치마크 (Task)
  ...
  * 🔄 매우 낮은 MAE & MSE 에 비교적 무거운 `Giga-Embeddings-instruct-480M-0826` 모델 사용하여, 경량 모델로 재선정 대상
...
| 🔄 변수명, 함수명은 의미가 있어야 함 (+ 알기 쉽게 할것)    | `LateOn-Code-pretrain`       | - Python 및 자연어의 2개 언어 역량을 동시에 평가<br>- 유사한 벤치마크인 `CodeSearchNetRetrieval` 보다 성능 측정값 누락 (missing value) 적음                                    |
...
| 🔄 함수의 인자가 하나로 묶을 수 있는 경우 처리 필요        | `LateOn-Code-pretrain`       | `변수명, 함수명은 의미가 있어야 함 (+ 알기 쉽게 할것)` 과 동일 **(자연어 맥락 고려 필요)**                                                                                  |
| 🔄 함수의 인자가 유동적인 경우 처리 필요               | `LateOn-Code-pretrain`       | 상동                                                                                                                                          |
| 🔄 상태 값으로 판단되는 값을 조건으로 하는지 여부          | `LateOn-Code-pretrain`       | 상동                                                                                                                                          |
...
| `CodeTransOceanContest`             | [Giga-Embeddings-instruct-480M-0826](https://huggingface.co/ai-sage/Giga-Embeddings-instruct-480M-0826) | - 유사한 변수명은 하나로 통일 시키는 것이 좋음<br>- 함수명과 반환값이 서로 잘 match 되어야 함<br>- 한 모듈 (*.py 파일) 내에서, 유사한 이름의 함수끼리 거리 검사                         |
| `CodeTransOceanContest` (경량 모델 재선정) | [LateOn-Code-pretrain](https://huggingface.co/lightonai/LateOn-Code-pretrain)                           | - 변수명, 함수명은 의미가 있어야 함 (+ 알기 쉽게 할것)<br>- 함수의 인자가 하나로 묶을 수 있는 경우 처리 필요<br>- 함수의 인자가 유동적인 경우 처리 필요<br>- 상태 값으로 판단되는 값을 조건으로 하는지 여부 |
