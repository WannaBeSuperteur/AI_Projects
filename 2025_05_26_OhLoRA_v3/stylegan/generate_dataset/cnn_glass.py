...
# labeling 이 안 된 13,000 장에 대해 예측된 Glass 속성 값 반환
...
# Returns:
# - final_score (Pandas DataFrame) : Glass 속성 값에 대한 모델 예측값을 저장한 Pandas DataFrame
#                                    columns = ['img_no', 'img_path', 'property_glass_final_score',
#                                               'score_model_0', 'score_model_1', ...]

def main_glass():

    # load dataset
    data_loader = load_dataset(property_name='glass')

    # load or train model
    try:
        print('loading CNN models ...')
        cnn_models = load_cnn_model(property_name='glass', cnn_model_class=GlassCNN)
        ...
        cnn_models = train_cnn_models(data_loader,
                                      is_stratified=True,
                                      property_name='glass',
                                      cnn_model_class=GlassCNN)

    # run inference on remaining 13,000 images
    remaining_image_loader = load_remaining_images_dataset(property_name='glass')
    report_path = f'{INFERENCE_RESULT_DIR}/glass.csv'
    os.makedirs(INFERENCE_RESULT_DIR, exist_ok=True)

    final_score = predict_score_remaining_images(property_name='glass',
                                                 remaining_images_loader=remaining_image_loader,
                                                 cnn_models=cnn_models,
                                                 report_path=report_path)

    print('FINAL PREDICTION SCORE (GLASS) :\n')
    print(final_score)
