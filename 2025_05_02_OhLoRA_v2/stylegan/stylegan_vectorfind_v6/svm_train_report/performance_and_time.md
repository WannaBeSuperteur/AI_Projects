...
| **t-SNE**                                            | $O(n)$                           |
| **SVM** (Support Vector Machine) - **linear** kernel | $O(n)$ ~ $O(n^2)$                |

* SVM implementation

```python
svm_clf = svm.SVC(kernel='linear', random_state=2025+i)  # w/o linearSVC
svm_clf = svm.LinearSVC(random_state=2025+i)             # w/ linearSVC
```

### 1-2. Speedup of ```sklearnex``` Library

* Sampling (image synthesize) 속도 **약간 느려짐**
* t-SNE 속도 **약간 느려짐**
* SVM training 속도 **약 1.5배 빨라짐**
  * SVC with linear kernel → **LinearSVC** 로 변경 시 **약 8배 빨라짐**
