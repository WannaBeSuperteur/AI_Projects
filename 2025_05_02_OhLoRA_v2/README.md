...
| 🔍 최종 검토 | 최종 사용자 실행용 코드 작성                                         | 05.12 월 (1d)           | ```P003-016-ForUser``` |       | ⬜  |
| 🔍 최종 검토 | 최종 QA (버그 유무 검사)                                         | 05.12 월 (1d)           |                        |       | ⬜  |
| 📃 문서화   | 프로젝트 문서 정리 및 마무리                                         | 05.12 월 (1d)           |                        |       | ⬜  |
|          | buffer time **(이력서 업데이트, 과제 전형, 면접 등 취업 일정에 대비한 여유 기간)** | 05.13 화 - 05.16 금 (4d) |                        |       |    |
...
| 구분         | 계획 내용                                                                                 | 일정                     | branch                                    | issue | 상태 |
|------------|---------------------------------------------------------------------------------------|------------------------|-------------------------------------------|-------|----|
| 📝 데이터셋 작성 | LLM Supervised Fine-Tuning 학습 데이터 증량                                                  | 05.09 금 (1d)           |                                           |       | ⬜  |
| 🧪 모델 학습   | [KoreamLM (by QuantumAI)](https://huggingface.co/quantumaikr/KoreanLM) Fine-Tuning 시도 | 05.09 금 (1d)           | ```P003-011-KoreanLM```                   |       | ⬜  |
| 📝 데이터셋 작성 | S-BERT (for memory) 학습 데이터 증량 및 품질 향상                                                 | 05.10 토 (1d)           |                                           |       | ⬜  |
| 🔨 모델 개선   | S-BERT (for memory) ground-truth similarity score 계산 고도화                              | 05.10 토 (1d)           | ```P003-012-SBERT-sim```                  |       | ⬜  |
| 🔨 모델 개선   | 하고 있는 대화 요약을 S-BERT (for memory) 와 연동                                                 | 05.10 토 (1d)           | ```P003-013-SBERT-current-conversation``` |       | ⬜  |              
| 🔨 모델 개선   | S-BERT (for memory) 임베딩 방법 개선 연구                                                      | 05.10 토 - 05.11 일 (2d) | ```P003-014-SBERT-embedding```            |       | ⬜  |
| ⚙ 기능 개선    | 답변 생성 시도 및 성공/실패 처리 메커니즘 개선                                                           | 05.11 일 (1d)           | ```P003-015-Answer```                     |       | ⬜  |
| ⚙ 기능 개선    | 실시간 표정 변화 적용                                                                          | 05.11 일 (1d)           | ```P003-016-Realtime```                   |       | ⬜  |
