...
| 🔍 최종 검토 | 최종 사용자 실행용 코드 작성                                         | 05.08 목 (1d)           | ```P003-016-ForUser``` |       | ⬜  |
| 🔍 최종 검토 | 최종 QA (버그 유무 검사)                                         | 05.09 금 (1d)           |                        |       | ⬜  |
| 📃 문서화   | 프로젝트 문서 정리 및 마무리                                         | 05.09 금 (1d)           |                        |       | ⬜  |
...
| 🔨 모델 개선 | 핵심 속성 값의 차이를 이용한 Loss 구현 (Generator 만 학습)                                                                                                          | 05.05 일 (1d)           | ```P003-008-StyleGAN-Loss```            | [issue](https://github.com/WannaBeSuperteur/AI_Projects/issues/1) | 💨 |
...
| 🧪 모델 학습 | StyleGAN 학습 **3차** (구조 추가 변경된 버전)                                                                                                                  | 05.05 월 (1d)           | ```P003-006-StyleGAN-Modify```          | [issue](https://github.com/WannaBeSuperteur/AI_Projects/issues/1) | ⬜  |
| 📕 논문    | **(추가)** StyleGAN 에서 핵심 속성 값만 변화시키는 vector 추출 방법 논문 탐독 ([참고 논문](https://arxiv.org/pdf/1911.09267))                                                 | 05.05 월 (1d)           |                                         |                                                                   | ⬜  |
| 🔨 모델 개선 | **(추가)** StyleGAN 에서 핵심 속성 값만 변화시키는 vector 추출 구현                                                                                                   | 05.05 월 - 05.06 화 (2d) | ```P003-009-StyleGAN-VectorFind```      |                                                                   | ⬜  |

**3. LLM 을 이용한 대화 능력 향상**

| 구분         | 계획 내용                                                                                 | 일정                     | branch                                    | issue | 상태 |
|------------|---------------------------------------------------------------------------------------|------------------------|-------------------------------------------|-------|----|
| 📝 데이터셋 작성 | LLM Supervised Fine-Tuning 학습 데이터 증량                                                  | 05.06 화 (1d)           |                                           |       | ⬜  |
...
| 🔨 모델 개선   | S-BERT (for memory) ground-truth similarity score 계산 고도화                              | 05.07 수 (1d)           | ```P003-011-SBERT-sim```                  |       | ⬜  |
