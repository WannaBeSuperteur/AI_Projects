...
GENERATED_IMG_PATH = f'{PROJECT_DIR_PATH}/v9_property_scores/generated_images'
...
# StyleGAN-FineTune-v9 모델의 Generator 로딩
...
# Returns:
# - finetune_v9_generator (nn.Module) : StyleGAN-FineTune-v9 모델의 Generator

def load_stylegan_finetune_v9_generator(device):
...
# StyleGAN-FineTune-v9 모델의 Generator 를 이용하여 이미지 생성
# Create Date : 2025.06.10
# Last Update Date : -
...
def generate_images(finetune_v9_generator, n=20000):
    for i in range(n):
        if i % 250 == 0:
...
def compute_property_scores(merged_property_score_cnn):
    ...
    with torch.no_grad():
        for img_no in range(image_cnt):
            if img_no % 500 == 0:
                print(f'compute score : {img_no} / {image_cnt}')
    ...
    score_df.to_csv(f'{PROJECT_DIR_PATH}/v9_property_scores/property_scores_sampled.csv',
                    index=False)
    ...
    mean_and_median_df.to_csv(f'{PROJECT_DIR_PATH}/v9_property_scores/property_scores_mean_and_median.csv',
                              index=False)


if __name__ == '__main__':
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f'device for generating image using StyleGAN-FineTune-v9 : {device}')

    finetune_v9_generator = load_stylegan_finetune_v9_generator(device)
    merged_property_score_cnn = load_merged_property_score_cnn(device)

    generate_images(finetune_v9_generator)
