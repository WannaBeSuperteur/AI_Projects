...
# 옴브레 염색 적용된 이미지를 이용한 움직이는 OpenCV 화면 생성
...
def create_opencv_screen(vectorfind_generator, hair_seg_model, vectorfind_ver, ohlora_no,
                         color_list, ombre_height_list, ombre_grad_height_list, pms_list):
    ...
    for idx in range(n_frames):
        ...
        ohlora_image = ohlora_image / 255.0
        cv2.imshow('Oh-LoRA', ohlora_image[:, :, ::-1])
        _ = cv2.waitKey(1)
