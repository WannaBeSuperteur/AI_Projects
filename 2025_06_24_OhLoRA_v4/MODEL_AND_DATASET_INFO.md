...
* 아래 표에 표시된 모든 모델 **(총 9개)** 은 **Oh-LoRA 👱‍♀️ (오로라) v4** 의 원활한 동작을 위해 필요
* 본 문서에 표시된 모든 저장 경로는 **처음부터 ```/2025_06_24_OhLoRA_v4/``` 까지의 경로는 제외하고 나타낸** 경로임.
* ```stylegan/models``` 및 ```segmentation/models``` 디렉토리가 처음 clone 받았을 때는 없으므로, **해당 디렉토리를 먼저 생성 후** 진행
...
| [Segmentation Model for Oh-LoRA v4](segmentation/README.md#1-segmentation-방법)                                                                              | Oh-LoRA 👱‍♀️ (오로라) 얼굴에 **Ombre 헤어스타일 적용**                                                                                                                                                                                                                      | [HuggingFace](https://huggingface.co/daebakgazua/250624_OhLoRA_Hair_Segmentation/tree/main) > ```segmentation_model_ohlora_v4.pth``` | ```segmentation/models``` 경로에 원래 이름 그대로 저장 | 
