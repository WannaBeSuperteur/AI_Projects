...
* 전체 일정 : **2025.10.06 월 - 10.11 토 + 2026.03.15 일 - 03.19 목**
...
| 🔍 최종 검토 | 최종 사용자 실행용 코드 작성           | 03.18 수 (1d) | ```P009-008-ForUser``` |       | ⬜  |
| 🔍 최종 검토 | 최종 QA (버그 유무 검사)           | 03.19 목 (1d) |                        |       | ⬜  |
| 📃 문서화   | 데이터셋 및 모델 HuggingFace 에 등록 | 03.19 목 (1d) |                        |       | ⬜  |
| 📃 문서화   | 프로젝트 문서 정리 및 마무리           | 03.19 목 (1d) |                        |       | ⬜  |
...
| 🧪 모델 학습   | 이미지의 hidden representation 모델 학습                      | 10.11 토 + 03.15 일 (2d) | ```P009-002-hidden-representation``` |                                                                    | ⬜  |
| ⚙ 기능 구현    | 학습 데이터 이미지 부분 추출 (hidden representation 기반)           | 03.15 일 (1d)           | ```P009-003-select-images```         |                                                                    | ⬜  |         
| 🔨 모델 구현   | 최적 하이퍼파라미터 탐색 모델 구현 **(hidden representation 입력 기반)** | 03.15 일 (1d)           | ```P009-004-hp-find-model```         |                                                                    | ⬜  |
| 🧪 모델 학습   | 최적 하이퍼파라미터 탐색 모델 학습                                   | 03.15 일 - 03.17 화 (3d) | ```P009-005-hp-find-model-train```   |                                                                    | ⬜  |
| 📃 문서화     | "하이퍼파라미터 튜닝 대결" 개발 내용 문서화                             | 03.17 화 (1d)           |                                      |                                                                    | ⬜  |

**3. LLM을 이용한 대결 결과 표현**

| 구분       | 계획 내용                                                | 일정           | branch                       | issue | 상태 |
|----------|------------------------------------------------------|--------------|------------------------------|-------|----|
| 🧪 모델 학습 | 대결 결과 표현 LLM Supervised Fine-Tuning 학습<br>(LLM: ???) | 03.17 화 (1d) | ```P009-006-train-LLM```     |       | ⬜  |
| ⚙ 기능 구현  | 대결 결과 리스트 기반 결과 표현 출력 구현                             | 03.18 수 (1d) | ```P009-007-battle-result``` |       | ⬜  |
| 📃 문서화   | "LLM을 이용한 대결 결과 표현" 개발 내용 문서화                        | 03.18 수 (1d) |                              |       | ⬜  |
