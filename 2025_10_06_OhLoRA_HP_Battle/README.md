...
| 🔍 최종 검토 | 최종 사용자 실행용 코드 작성           | 10.11 토 (1d) | ```P009-006-ForUser``` |       | ⬜  |
| 🔍 최종 검토 | 최종 QA (버그 유무 검사)           | 10.12 일 (1d) |                        |       | ⬜  |
| 📃 문서화   | 데이터셋 및 모델 HuggingFace 에 등록 | 10.12 일 (1d) |                        |       | ⬜  |
| 📃 문서화   | 프로젝트 문서 정리 및 마무리           | 10.12 일 (1d) |                        |       | ⬜  |

**2. 하이퍼파라미터 튜닝 대결**

| 구분         | 계획 내용                                                 | 일정                     | branch                               | issue | 상태 |
|------------|-------------------------------------------------------|------------------------|--------------------------------------|-------|----|
| 📝 데이터셋 선택 | 인간과의 하이퍼파라미터 대결을 할 **이미지 분류 데이터셋** 선정                 | 10.07 화 (1d)           |                                      |       | ⬜  |
| 🔨 모델 구현   | 기본 CNN 모델 (하이퍼파라미터 튜닝 대상) 구현                          | 10.07 화 (1d)           | ```P009-001-CNN```                   |       | ⬜  |
| 🔨 모델 구현   | 이미지의 hidden representation 모델 구현 (Auto-Encoder?)      | 10.08 수 (1d)           | ```P009-002-hidden-representation``` |       | ⬜  |
| 🧪 모델 학습   | 이미지의 hidden representation 모델 학습                      | 10.08 수 (1d)           | ```P009-002-hidden-representation``` |       | ⬜  |
| 🔨 모델 구현   | 최적 하이퍼파라미터 탐색 모델 구현 **(hidden representation 입력 기반)** | 10.08 수 (1d)           | ```P009-003-hp-find-model```         |       | ⬜  |
| 🧪 모델 학습   | 최적 하이퍼파라미터 탐색 모델 학습                                   | 10.08 수 - 10.10 금 (3d) | ```P009-003-hp-find-model```         |       | ⬜  |
| 📃 문서화     | "하이퍼파라미터 튜닝 대결" 개발 내용 문서화                             | 10.10 금 (1d)           |                                      |       | ⬜  |

**3. LLM을 이용한 대결 결과 표현**

| 구분       | 계획 내용                                                | 일정           | branch                       | issue | 상태 |
|----------|------------------------------------------------------|--------------|------------------------------|-------|----|
| 🧪 모델 학습 | 대결 결과 표현 LLM Supervised Fine-Tuning 학습<br>(LLM: ???) | 10.10 금 (1d) | ```P009-004-train-LLM```     |       | ⬜  |
| ⚙ 기능 구현  | 대결 결과 리스트 기반 결과 표현 출력 구현                             | 10.11 토 (1d) | ```P009-005-battle-result``` |       | ⬜  |
| 📃 문서화   | "LLM을 이용한 대결 결과 표현" 개발 내용 문서화                        | 10.11 토 (1d) |                              |       | ⬜  |
