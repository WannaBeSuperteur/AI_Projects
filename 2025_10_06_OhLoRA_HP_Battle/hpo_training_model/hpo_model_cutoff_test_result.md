...
* [3. 테스트 결과](#3-테스트-결과)
  ...
  * [3-4. Option 3 테스트 결과 (데이터셋 확대 이후, 최종)](#3-4-option-3-테스트-결과-데이터셋-확대-이후-최종)
* [4. 테스트 결과에 따른 결정](#4-테스트-결과에-따른-결정)
  ...
  * [4-4. Option 3 테스트 결과 (데이터셋 확대 이후, 최종)](#4-4-option-3-테스트-결과-데이터셋-확대-이후-최종)
...
|----------|------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Option 1 | ```{base_dir}/hpo_model_train_dataset_df_*.csv```    | - [```hpo_model_test_result_per_corr_threshold_cutoff.csv```](hpo_model_test_result_per_corr_threshold_cutoff.csv) (threshold 0.0 - 0.3)<br>- [```hpo_model_test_result_per_corr_threshold_cutoff_2.csv```](hpo_model_test_result_per_corr_threshold_cutoff_2.csv) (threshold 0.3 - 0.6)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| L       | [```hpo_model_test_result_per_corr_threshold_cutoff_new2_6.csv```](hpo_model_test_result_per_corr_threshold_cutoff_new2_6.csv) | 15,600                      | 15,600                           | 15,600                   | FC 3개            | 미 적용                                 | 미 적용                | 
| L2      | [```hpo_model_test_result_per_corr_threshold_cutoff_new2_7.csv```](hpo_model_test_result_per_corr_threshold_cutoff_new2_7.csv) | 20,000                      | 20,000                           | 20,000                   | FC 3개            | 미 적용                                 | 미 적용                | 
| L3      | [```hpo_model_test_result_per_corr_threshold_cutoff_new2_8.csv```](hpo_model_test_result_per_corr_threshold_cutoff_new2_8.csv) | 24,000                      | 24,000                           | 24,000                   | FC 3개            | 미 적용                                 | 미 적용                | 
...
### 3-4. Option 3 테스트 결과 (데이터셋 확대 이후, 최종)

| 데이터셋          | Corr-coef 테스트 결과                     |
|---------------|--------------------------------------|
| CIFAR-10      | ![image](../../images/251006_23.png) |
| Fashion MNIST | ![image](../../images/251006_24.png) |
| MNIST         | ![image](../../images/251006_25.png) |
...
### 4-4. Option 3 테스트 결과 (데이터셋 확대 이후, 최종)
...
* 근거
  * [Option 3 테스트 결과 (데이터셋 확대 이후)](#3-4-option-3-테스트-결과-데이터셋-확대-이후-최종) 에 따른 **확대 데이터셋 (L2) 기준** 최대 corr-coef 값이 되는 threshold cutoff
  * mnist 데이터셋의 경우 0.40 에서 corr-coef 가 최대이지만, 해당 cutoff가 측정이 이루어진 최대 cutoff 이므로 그 주변에서 변동성이 있을 것으로 예상
