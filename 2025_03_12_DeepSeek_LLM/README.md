...
| LLM Fine-tuning 의 Flow-Chart 학습 데이터 다양화      | 03.20 목 (1d)           | ```P001-007-UpdateFlowchart``` | ✅  |
| LLM Fine-tuning 실시 (ORPO)                    | 03.22 토 - 03.23 일 (2d) | ```P001-006-FineTune```        | ❌  |
| CNN 개발 및 학습                                  | 03.23 일 (1d)           | ```P001-008-CNN```             | 💨 |
| Auto-Encoder 개발 및 학습                         | 03.23 일 (1d)           | ```P001-009-AE```              | ⬜  |
| k-NN 개발 및 학습                                 | 03.23 일 (1d)           | ```P001-010-kNN```             | ⬜  |
| 기본 가독성 + 예상 사용자 평가 점수 처리 알고리즘 개발             | 03.24 월 (1d)           | ```P001-011-Score```           | ⬜  |
...
|-------------------------------------------------------------------------------------------------------------------------------------------------------|------------|--------|-----------|-----------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| ```flash_attn``` 사용 불가                                                                                                                                | 2025.03.14 | 낮음     | 보류        | ```nvcc -V``` 기준의 CUDA 버전 이슈                              | - Windows 환경 변수 편집 **(해결 안됨)**<br>- flash_attn 라이브러리의 이전 버전 설치 **(실패)**<br>- Visual C++ 14.0 설치 **(실패)**                                                                                                                                                                  |
| LLM 출력이 매번 동일함                                                                                                                                        | 2025.03.15 | 보통     | 해결 완료     | ```llm.generate()``` 함수의 랜덤 생성 인수 설정 누락                   | - ```torch.manual_seed()``` 설정 **(해결 안됨)**                                                                                                                                                                                                                                |
| 다이어그램 이미지가 overwrite 됨                                                                                                                                | 2025.03.18 | 보통     | 해결 완료     | 텍스트 파싱 및 도형 그리기 알고리즘의 **구현상 이슈**                          | - 일정 시간 간격으로 다이어그램 생성 **(해결 안됨)**<br>- ```canvas.copy()``` 이용 **(해결 안됨)**<br>- garbage collection 이용 **(해결 안됨)**                                                                                                                                                          |
...
* **문제 해결 부분적 성공 시까지 시도한 부분**
  * Fully-Connected Layer 의 [Dropout](https://github.com/WannaBeSuperteur/AI-study/blob/main/AI%20Basics/Deep%20Learning%20Basics/%EB%94%A5%EB%9F%AC%EB%8B%9D_%EA%B8%B0%EC%B4%88_Overfitting_Dropout.md#3-dropout) 제거
    * 기본 아이디어 : Conv Layer 에 Dropout 을 적용하면 Auto-Encoder 학습이 어려워질 수 있는데, Fully-Connected 도 마찬가지일 수 있다 
    * 결과 : **해결 안됨**
