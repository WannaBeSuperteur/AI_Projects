...
# - accuracy 외에 "(정답) - (유사도 점수가 가장 높은 오답)" 값의 평균을 성능지표로 추가
...
# - mean_diff_correct_and_max_sim_wrong (float)     : "(정답) - (유사도 점수가 가장 높은 오답)" 값의 평균
...
def read_file_and_compute_accuracy(file_path, output_part_column):
    ...
    sum_diff_correct_and_max_sim_wrong = 0.0
    wrong_answer_info = []
    ...
    for input_part_distinct in input_part_distinct_list:
        ...
        if predicted_max_idx == gt_max_idx:
            ...
            sim_correct = filtered_data[f'predicted_{similarity_suffix}'].max()
            max_sim_wrong = sorted(filtered_data[f'predicted_{similarity_suffix}'].tolist(), reverse=True)[1]
            ...
            sim_correct = filtered_data[f'predicted_{similarity_suffix}'].tolist()[gt_max_idx]
            max_sim_wrong = filtered_data[f'predicted_{similarity_suffix}'].max()

        sum_diff_correct_and_max_sim_wrong += sim_correct - max_sim_wrong

    accuracy = round(correct / total, 4)
    mean_diff_correct_and_max_sim_wrong = round(sum_diff_correct_and_max_sim_wrong / total, 4)

    return accuracy, mean_diff_correct_and_max_sim_wrong, wrong_answer_info
