...
def load_fine_tuned_llm(llm_name):
    ...
    if llm_name == 'kanana':
        ...
        fine_tuned_llm = AutoModelForCausalLM.from_pretrained(
            f'{PROJECT_DIR_PATH}/ai_quiz/models/kananai_sft_final_fine_tuned_20epochs',
            trust_remote_code=True,
            torch_dtype=torch.bfloat16).cuda()
...
def inference_or_fine_tune_llm(llm_name):
    ...
    inference_log_path = f'{llm_log_path}/{llm_name}_sft_final_inference_log_{inference_temperature}_20epochs.txt'
    inference_log = ''
