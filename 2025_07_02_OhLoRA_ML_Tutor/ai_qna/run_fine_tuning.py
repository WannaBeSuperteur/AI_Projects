...
def load_fine_tuned_llm(llm_name):
    ...
    if llm_name == 'kanana':
        fine_tuned_llm = AutoModelForCausalLM.from_pretrained(
            f'{PROJECT_DIR_PATH}/ai_qna/models/kanana_sft_wo_rag_fine_tuned',
            trust_remote_code=True,
            torch_dtype=torch.bfloat16).cuda()

    elif llm_name == 'kananai':
        fine_tuned_llm = AutoModelForCausalLM.from_pretrained(
            f'{PROJECT_DIR_PATH}/ai_qna/models/kananai_sft_wo_rag_fine_tuned',
            trust_remote_code=True,
            torch_dtype=torch.bfloat16).cuda()
...
def inference_or_fine_tune_llm(llm_name):
    ...
    try:
        ...
        tokenizer = AutoTokenizer.from_pretrained(f'{models_dir}/{llm_name}_sft_wo_rag_fine_tuned')
    ...
    inference_log_path = f'{llm_log_path}/{llm_name}_sft_wo_rag_inference_log_{inference_temperature}.txt'
    inference_log = ''
